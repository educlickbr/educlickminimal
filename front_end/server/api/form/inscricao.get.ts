...
export default defineEventHandler(async (event) => {
  ...
  const { id_processo, tipo_proc, tipo_cand } = getQuery(event) as {
    id_processo?: string
    tipo_proc?: string
    tipo_cand?: string
  }
  ...
  if (!id_processo || !tipo_proc || !tipo_cand) {
    return { success: false, message: 'Parâmetros incompletos (id_processo, tipo_proc, tipo_cand)', inscricao: null }
  }
