...
export default defineEventHandler(async (event) => {
  ...
  const saveToDb = body.find((item) => item.name === 'saveToDb')?.data?.toString() === 'true';
  const escopo = body.find((item) => item.name === 'escopo')?.data?.toString() || 'geral';
  ...
  try {
    ...
    throw createError({ statusCode: 500, message: 'Erro ao enviar arquivo para o R2.' });
  }
  ...
  if (saveToDb) {
    try {
      ...
      throw createError({ statusCode: 500, message: 'Arquivo subiu pro R2, mas erro ao salvar no banco: ' + dbErr.message });
    }
  }
