...
export default defineEventHandler(async (event) => {
  ...
  if (!id_entidade || !ciclos || ciclos.length === 0) {
    return {
      success: false,
      message: "Dados incompletos para geração do programa.",
    };
  }
  ...
  const { data, error } = await client.rpc("aca_create_programas_lote", {
    ...
    p_gratuito: gratuito !== undefined ? gratuito : null,
    p_exige_processo_seletivo:
      exige_processo_seletivo !== undefined ? exige_processo_seletivo : null,
  } as any);
