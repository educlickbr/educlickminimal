    ...
    <div class="modal-panel">
      ...
      <div class="modal-header">
        <div class="modal-header-icon">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 256 256"><path d="M117.25,157.92a60,60,0,1,0-66.5,0A95.83,95.83,0,0,0,3.53,195.63a8,8,0,1,0,13.4,8.74,80,80,0,0,1,134.14,0,8,8,0,0,0,13.4-8.74A95.83,95.83,0,0,0,117.25,157.92Z"/></svg>
        </div>
        <div class="modal-header-text">
          <h3 class="modal-title">{{ isEdit ? 'Editar' : 'Novo' }} Programa</h3>
          <p class="modal-subtitle">Oferta Acadêmica / Turma</p>
        </div>
      ...
      <div class="p-6 flex flex-col gap-5">
        <div class="flex flex-col gap-2">
          ...
          <p v-if="isEdit" class="text-[9px] text-secondary/30 italic px-1 font-bold">O curso base não pode ser alterado após a criação.</p>
        </div>

        <div class="flex flex-col gap-2">
          <label class="text-[10px] font-black text-secondary/60 uppercase tracking-widest px-1">Identificação / Descrição da Turma</label>
          <input 
            v-model="form.descricao" 
            placeholder="Ex: Turma 2024.1 - Noturno"
            class="w-full px-4 py-3 rounded-lg border border-secondary/10 bg-background text-sm font-bold text-primary focus:border-primary/50 transition-all outline-none" 
          />
        </div>

        <div class="bg-primary/5 border border-primary/10 rounded-lg p-4 flex gap-3 items-start">
          <Icon name="ph:info-bold" class="w-4 h-4 text-primary shrink-0 mt-0.5" />
          <div class="flex flex-col">
            <p class="text-[10px] font-bold text-primary uppercase tracking-widest leading-tight">Dica de Gestão</p>
            <p class="text-[11px] text-secondary/70 font-medium">Após salvar o programa, você poderá configurar os ciclos e o calendário para cada módulo na aba específica de Ciclos.</p>
          </div>
        </div>
      </div>
      ...
      <div class="modal-footer">
        ...
        <button @click="handleSave" :disabled="loading" class="modal-btn-save">
...
async function handleSave() {
  if (!form.id_curso || !form.descricao.trim()) {
    return toast.showToast('Curso e Descrição são obrigatórios', { type: 'error' })
  }
  ...
  try {
    ...
    if (res?.success) {
      ...
      throw new Error(res?.message || 'Erro ao salvar programa')
    }
  } catch (e: any) {
...
watch(() => props.modelValue, (val) => {
  if (val) {
    if (props.initialData) {
      form.id_curso = props.initialData.id_curso || null
