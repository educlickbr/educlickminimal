  ...
  <div class="flex flex-col gap-6">
    ...
    <div v-else class="flex flex-col gap-6">
      <div class="p-5 rounded-xl border border-primary/10 bg-primary/5 flex flex-col gap-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div class="flex flex-col gap-1.5 md:col-span-2">
            <BaseField v-model="formMC.id_componente" type="select" empty-label="Selecione o componente" :options="componentesDisponiveis" optionValueKey="id" optionLabelKey="nome_componente" label="Componente Curricular" />
          </div>
          <div class="flex flex-col gap-1.5">
            <label class="text-[9px] font-black text-secondary/60 uppercase tracking-widest px-1">Carga Horária (h:m)</label>
            <div class="flex items-center gap-1">
              <BaseField v-model="formMC.horas" placeholder="00" type="number" min="0" class="flex-1" />
              <span class="text-secondary/40 font-bold">:</span>
              <BaseField v-model="formMC.minutos" placeholder="00" type="number" min="0" max="59" class="flex-1" />
            </div>
          </div>
        </div>
        <div class="flex items-center justify-between gap-4">
          ...
          <button @click="$emit('addComponente')" :disabled="loadingAddComponente || !formMC.id_componente" class="ds-btn-primary">{{ loadingAddComponente ? "Adicionando..." : "Adicionar" }}</button>
        </div>
      </div>
      <div class="flex flex-col gap-3">
        ...
        <div v-else-if="componentesDoModulo.length === 0" class="ds-empty">Nenhum componente associado a este módulo.</div>
        <div v-for="(mc, index) in componentesDoModulo" :key="mc.id_componente" class="flex items-center gap-0 rounded-lg border border-divider bg-div-15 group hover:border-primary/20 transition-all overflow-hidden">
          <div class="w-10 shrink-0 h-full flex items-center justify-center bg-div-30 border-r border-divider"><span class="text-[10px] font-black text-secondary/30 tabular-nums">{{ index + 1 }}</span></div>
