    ...
    <div class="flex flex-col gap-6">
                ...
                <div class="flex flex-col gap-1.5 justify-end">
                    <div class="flex gap-2">
                        <button
                            @click="$emit('saveArea')"
                            :disabled="
                                loadingArea || !formArea.nome_area.trim()
                            "
        ...
        <div class="flex flex-col gap-3">
            ...
            <div
                ...
                :key="a.id"
                class="flex items-center justify-between px-4 py-3 rounded-lg border border-white/5 bg-white/[0.03] group hover:border-primary/20 transition-all"
            >
                ...
                <div class="flex items-center gap-2">
                    <button
                        @click="$emit('editArea', a)"
                        class="p-1.5 text-secondary/30 hover:text-primary transition-all"
                    >
                        <Icon name="ph:pencil-simple-bold" />
                    </button>
                    <button
                        @click="$emit('deleteArea', a.id)"
                        class="p-1.5 text-secondary/30 hover:text-red-400 transition-all"
                    >
