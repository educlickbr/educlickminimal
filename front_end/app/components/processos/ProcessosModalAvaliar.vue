    ...
    <div v-if="modelValue" class="modal-overlay" @click.self="fechar">
        <div class="modal-panel">
            ...
            <div class="modal-body flex flex-col gap-4">
                <p class="text-[10px] text-secondary/60 uppercase tracking-wider">
                    Clique no botão para alternar entre Pendente e Aprovado
                </p>

                <!-- Dados -->
                <div class="flex items-center justify-between bg-div-15 border border-divider rounded-xl p-4">
...
.modal-panel {
    ...
    border: 1px solid var(--color-divider); border-radius: 16px;
    width: 100%; max-width: 480px; overflow: hidden;
    display: flex; flex-direction: column;
    box-shadow: 0 24px 80px rgba(0, 0, 0, 0.3), 0 0 0 1px rgba(139, 92, 246, 0.1);
...
.modal-close-btn:hover { background: var(--color-secondary-surface-hover); color: var(--color-text); }
...
.modal-footer {
    ...
    background: var(--color-secondary-surface); flex-shrink: 0;
}
...
.modal-btn-cancel:hover { background: var(--color-secondary-surface-hover); color: var(--color-text); }
</style>
