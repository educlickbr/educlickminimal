...
function getFormUrl(prog: any) {
    ...
    // Se o usuário não está logado, manda para o login com redirectTo
    if (!user.value) {
        return `/login?redirectTo=${encodeURIComponent(fullUrl)}`;
    }
...
<template>
    <div
        class="min-h-screen bg-[#0a0a0c] text-white font-sans selection:bg-primary/30"
    >
        <!-- Custom Public Header -->
        <header
            class="sticky top-0 z-50 bg-[#0f0f17]/80 backdrop-blur-md border-b border-white/5 px-6 py-4 flex items-center justify-between"
        >
            <div class="flex items-center gap-3">
                <div
                    class="w-10 h-10 rounded-xl bg-gradient-to-br from-primary to-primary-dark flex items-center justify-center shadow-lg shadow-primary/20"
                >
                    <Icon
                ...
                <div class="flex flex-col leading-tight">
                    <span
                        class="text-sm font-black uppercase tracking-[0.2em] text-primary"
                        >EduClick</span
                    >
                    <span
                        class="text-[10px] font-bold text-secondary uppercase tracking-widest opacity-60"
            ...
            <nav class="hidden md:flex items-center gap-8">
                ...
                <NuxtLink
                    to="/oferta"
                    class="text-xs font-bold uppercase tracking-widest text-primary transition-colors border-b-2 border-primary pb-1"
                    >Programas</NuxtLink
                >
                <a
                    href="#"
            ...
            <NuxtLink
                v-else
                to="/"
                class="px-5 py-2 rounded-xl bg-primary/10 border border-primary/20 text-[10px] font-black text-primary uppercase tracking-widest hover:bg-primary/20 transition-all"
            >
                Minha Conta
            </NuxtLink>
        </header>
        ...
        <section class="relative py-20 px-6 overflow-hidden">
            ...
            ></div>
            <div
                class="absolute bottom-[-10%] right-[-10%] w-[30%] h-[30%] bg-secondary/5 blur-[100px] rounded-full"
            ></div>

            <div class="max-w-6xl mx-auto relative z-10 text-center">
                <span
                    class="inline-block px-4 py-1.5 rounded-full bg-primary/10 border border-primary/20 text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-6 animate-bounce"
                >
                    Inscrições Abertas
                </span>
                ...
                <p
                    class="text-lg text-secondary max-w-2xl mx-auto leading-relaxed opacity-80"
                >
                    Explore nossos programas acadêmicos e trilhas de formação.
                    Processos seletivos simplificados para você dar o próximo
        ...
        <main class="max-w-7xl mx-auto px-6 pb-20">
            ...
            >
                <div
                    class="flex items-center gap-2 bg-white/5 p-1 rounded-xl border border-white/5 overflow-x-auto max-w-full"
                >
                    <button
                ...
                <div
                    class="text-[10px] font-bold text-secondary uppercase tracking-widest opacity-40"
                >
            ...
            >
                <div
                    v-for="i in 6"
                    :key="i"
                    class="h-[400px] rounded-xl bg-white/5 animate-pulse border border-white/5"
                ></div>
            </div>

            <div
                v-else-if="filteredProgramas.length === 0"
                class="py-20 text-center border-2 border-dashed border-white/5 rounded-2xl"
            >
                <Icon
                        ...
                        <div class="flex items-center justify-between mb-6">
                            ...
                            >
                                ...
                                <span class="text-[10px] font-bold"
                                    >{{ prog.carga_horaria_total_horas }}h
                                    Totais</span
                                >
                            </div>
                        </div>
        ...
        <footer class="py-12 border-t border-white/5 text-center bg-[#0a0a0c]">
            <div class="flex items-center justify-center gap-3 mb-6">
                ...
                />
                <span
                    class="text-sm font-black uppercase tracking-[0.3em] text-white"
                    >EduClick</span
                >
            </div>
            <p
                class="text-[10px] font-bold text-secondary uppercase tracking-[0.2em] opacity-40"
            >
