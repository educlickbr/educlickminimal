...
const docenteId = computed(() => route.params.id as string);
...
onMounted(async () => {
    ...
    try {
        const res = (await $fetch("/api/public/calendario/docente", {
            params: { id_docente: docenteId.value },
        })) as any;

        if (res?.success) {
            docente.value = res.docente;
            aulas.value = res.aulas || [];
        } else {
            error.value = res?.message || "Erro ao carregar calendário público do professor.";
        }
    } catch (e: any) {
    ...
    <div class="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8 font-sans">
        <div class="max-w-4xl mx-auto flex flex-col gap-6">
            ...
            <div class="flex items-center justify-between border-b border-slate-800 pb-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400">Agenda Pública do Professor</p>
                    </div>
                </div>
            </div>
            ...
            <div v-if="loading" class="py-20 flex flex-col items-center justify-center gap-3">
                ...
                <span class="text-xs font-bold text-slate-400">Carregando agenda...</span>
            </div>
            ...
            <div v-else class="flex flex-col gap-6">
                <div class="p-6 rounded-2xl bg-slate-900 border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h2 class="text-2xl font-bold text-white">Prof. {{ docente?.nome || 'Docente' }}</h2>
                        <p class="text-xs text-purple-400 font-mono mt-1">{{ docente?.email || '' }}</p>
                    </div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="flex flex-col gap-0.5">
                                ...
                                <p class="text-xs text-slate-400">
                                    Turma/Ciclo: <span class="text-slate-300 font-medium">{{ aula.ciclo || '-' }}</span>
                                </p>
                            </div>
                        </div>
                        <div>
