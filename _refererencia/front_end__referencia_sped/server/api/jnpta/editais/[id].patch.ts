...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        const { data, error } = await client.rpc("nxt_jnpta_editais_atualizar", {
            p_id: id,
            p_edital_titulo: body?.edital_titulo,
            p_edital_descricao: body?.edital_descricao,
            p_arquivo_edital: body?.arquivo_edital ?? null,
            p_ano_semestre: body?.ano_semestre,
            p_qual_tempo: body?.qual_tempo || null,
            p_dt_inicio: body?.dt_inicio,
            p_dt_fim: body?.dt_fim,
            p_publicado: body?.publicado ?? null,
        }) as any;

        if (error) throw createError({ statusCode: 500, statusMessage: error.message });
        if (!data?.ok) throw createError({ statusCode: 400, statusMessage: data?.erro || "Falha ao atualizar edital" });
