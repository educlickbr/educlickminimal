...
export default defineEventHandler(async (event) => {
    ...
    const { p_id_turma, p_id_pergunta, p_resposta, p_respostas_batch, p_user_expandido_id } = body;
    ...
    try {
        ...
        const { data, error } = await (client as any).rpc(
            "nxt_salvar_respostas_usuario",
            {
                ...
                p_user_expandido_id: p_user_expandido_id || null, // Pass explicit ID if provided
            },
        );
