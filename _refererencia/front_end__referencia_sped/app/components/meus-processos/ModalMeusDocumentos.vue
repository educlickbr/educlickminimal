...
// Pendentes primeiro, depois enviados (cada grupo mantém a ordem original)
const sortedFormData = computed(() => {
    const pending = formData.value.filter((q) => !answers.value[q.id_pergunta]);
    const sent = formData.value.filter((q) => answers.value[q.id_pergunta]);
    return { pending, sent, hasBoth: pending.length > 0 && sent.length > 0 };
});
...
const fetchData = async () => {
    ...
    try {
        const data = await ofetch<any[]>("/api/aluno/documentos", {
            params: {
        ...
        formData.value.forEach((q: any) => {
            if (q.resposta || q.arquivo_original) {
                fileNames.value[q.id_pergunta] =
                    q.arquivo_original || "Arquivo Enviado";
...
const performDeleteFile = async (question: any) => {
    ...
    try {
        await ofetch("/api/aluno/delete-file", {
            method: "POST",
            body: {
                id_turma: props.processo.id_turma,
                id_pergunta: question.id_pergunta,
...
const handleSaveAnswer = async (question: any) => {
    ...
    try {
        await ofetch("/api/aluno/save-answer", {
            method: "POST",
            body: {
                id_pergunta: question.id_pergunta,
...
<template>
    ...
    >
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
            ...
            >
                <div
                    class="relative transform overflow-hidden rounded-none md:rounded-xl bg-[#16161E] border-x-0 border-y-0 md:border md:border-white/10 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-3xl p-4 md:p-6"
                    @click.stop
                >
                    <div
                    ...
                    >
                        <!-- Helper Text -->
                        <div
                            class="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 md:p-4 mb-2 md:mb-4"
                        >
                            <p class="text-[10px] md:text-xs text-blue-200">
                                Envie os documentos solicitados abaixo. Os
                                arquivos devem estar em formato PDF (exceto a
                                foto, que pode ser JPG/PNG). Certifique-se de
                                que estão legíveis.
                            </p>
                        </div>

                        <!-- ─── SEÇÃO PENDENTES ────────────────────────────────────── -->
                        <template v-if="sortedFormData.pending.length > 0">
                            <div class="flex items-center gap-3">
                                ...
                                <span
                                    class="text-[10px] font-bold text-primary/60"
                                    >{{ sortedFormData.pending.length }} item{{
                                        sortedFormData.pending.length !== 1
                                            ? "s"
                                            : ""
                                    }}</span
                                >
                        ...
                        <div
                            v-for="question in sortedFormData.pending"
                            :key="'p-' + question.id_pergunta"
                            class="flex flex-col gap-1.5 md:gap-2 ring-1 ring-primary/20 rounded-xl p-3 -mx-1 bg-primary/5"
                        >
                            <label
                                class="text-xs md:text-sm font-bold text-white flex justify-between items-center"
                            >
                                <span>
                                    {{ question.label }}
                                    <span
                                ...
                                <!-- Badge Corrigir: vermelho (primary), igual à instrução lateral -->
                                <span
                                    class="text-[10px] font-black uppercase tracking-widest text-primary bg-primary/10 border border-primary/30 px-2 py-0.5 rounded-full flex-shrink-0"
                                >
                                    Corrigir
                                </span>
                            </label>
                            ...
                            <div
                                ...
                                class="relative border-2 border-dashed border-primary/20 rounded-xl p-4 transition-all hover:border-primary/40 hover:bg-primary/5 group text-center cursor-pointer"
                            ...
                            >
                                ...
                                >
                                    <div
                                        class="animate-spin rounded-full h-6 w-6 border-t-2 border-primary mb-2"
                                    ></div>
                        ...
                        <!-- ─── SEPARADOR ─────────────────────────────────────────── -->
                        <template v-if="sortedFormData.hasBoth">
                            <div class="flex items-center gap-3 pt-2">
                                ...
                                >
                                <div
                                    class="flex-1 h-px bg-emerald-500/20"
                                ></div>
                                <span
                                    class="text-[10px] font-bold text-emerald-400/60"
                                    >{{ sortedFormData.sent.length }} item{{
                                        sortedFormData.sent.length !== 1
                                            ? "s"
                                            : ""
                                    }}</span
                                >
                        ...
                        <!-- ─── SEÇÃO ENVIADOS ─────────────────────────────────────── -->
                        <div
                            v-for="question in sortedFormData.sent"
                            :key="'s-' + question.id_pergunta"
                            class="flex flex-col gap-1.5 md:gap-2"
                        >
                            <label
                                class="text-xs md:text-sm font-bold text-white flex justify-between items-center"
                            >
                                ...
                                <span
                                    class="text-xs font-bold text-emerald-400 flex items-center gap-1 flex-shrink-0"
                                >
                                    <svg
                                        class="w-3 h-3"
                                        fill="none"
                            ...
                            <div
                                v-if="question.tipo === 'arquivo'"
                                class="relative border-2 border-dashed border-emerald-500/30 bg-emerald-500/5 rounded-xl p-4 transition-all hover:border-primary/40 hover:bg-primary/5 group text-center cursor-pointer"
                                @click="triggerFileUpload(question.id_pergunta)"
                            >
                                <input
                                    :id="'file-' + question.id_pergunta"
                                ...
                                <div
                                    v-else
                                    class="flex items-center justify-between w-full py-1 px-2"
                                >
                                    <div
                                        v-if="
                                            !confirmDeletes[
                                        ...
                                        "
                                        class="flex items-center gap-3 flex-grow overflow-hidden"
                                    >
                                        <div
                                        ...
                                        <span
                                            ...
                                            >{{
                                                fileNames[question.id_pergunta]
                                            }}</span
                                        >
                                    </div>
                                    <div
                                    ...
                                    >
                                        <a
                                            ...
                                            :href="
                                                appStore.hash_base
                                                    ? `${appStore.hash_base}/${answers[question.id_pergunta]}`
                                                    : '#'
                                            "
                                            target="_blank"
                                    ...
                                    <div
                                        v-else
                                        class="w-full flex items-center justify-between bg-red-500/10 rounded-lg p-2 px-3"
                                    >
                                        <span
