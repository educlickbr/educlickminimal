...
const initModal = () => {
    ...
    if (props.existingReserva) {
        ...
        tipo.value =
            props.existingReserva.tipo_calculado === "turma"
                ? "curso"
                : "evento";
...
const fetchTurmas = async () => {
    ...
    try {
        const data = await ofetch("/api/producao/calendario/turmas", {
            params: {
        ...
        // Combine all lists
        const all = [
            ...((data as any).em_andamento || []),
            ...((data as any).encerrados || []),
        ];

        turmasOptions.value = all.map((t: any) => ({
            id: t.id_turma,
            nome: t.nome_curso_turno,
        }));
    } catch (e: any) {
...
const checkConflict = (targetScope: "horario" | "periodo" | "dia") => {
    ...
    if (targetScope === "horario") slotsToCheck = [props.slotData];
        ...
        slotsToCheck = props.allHorarios.filter(
            (h) =>
    ...
    const exList = props.allReservas.filter((r) => r.data === dayStr);
    ...
    for (const slot of slotsToCheck) {
        ...
        if (conflict) {
            ...
            if (
                !props.existingReserva ||
                conflict.id !== props.existingReserva.id
            ) {
...
const save = async () => {
    ...
    try {
        ...
        await ofetch("/api/producao/calendario/reservas", {
            method: "POST",
            body: {
                reservas: payload,
...
const confirmRemove = async () => {
    ...
    try {
        await ofetch("/api/producao/calendario/reservas", {
            method: "DELETE",
            params: { id: props.existingReserva.id },
        });
...
<template>
    ...
    >
        ...
        <div
            class="relative bg-[#1A1B26] border border-white/10 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]"
        >
            <!-- Header -->
            <div
            ...
            >
                <div>
                    ...
                    <p class="text-xs text-secondary">
                        {{ slotData?.nome }} - {{ format(day, "dd/MM/yyyy") }} -
                        {{ slotData?.horario_total }}
                    </p>
                </div>
                <button
                    @click="emit('close')"
            ...
            <div class="p-6 overflow-y-auto flex-1 custom-scrollbar space-y-6">
                ...
                <!-- Observações -->
                <div>
                    <label class="block text-xs font-bold text-secondary mb-2"
                        >Observações</label
                    >
                    <textarea
                        v-model="observacoes"
                ...
                <!-- Scope Selection -->
                <div>
                    <label class="block text-xs font-bold text-secondary mb-3"
                        >Aplicar Reserva para:</label
                    >
                    <div class="space-y-2">
                        <!-- Horario -->
                        <label
                            class="flex items-center justify-between p-3 rounded-lg border border-white/5 cursor-pointer hover:bg-white/5 transition-colors"
                            :class="
                                scope === 'horario'
                        ...
                        >
                            <div class="flex items-center gap-3">
                                <input
                                    ...
                                    v-model="scope"
                                    value="horario"
                                    class="text-primary focus:ring-primary bg-transparent border-white/20"
                                />
                                <span class="text-sm font-medium text-white"
                                    >Apenas este horário</span
                                >
                            </div>
                        ...
                        >
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    >
                                    <span
                                        v-if="isPeriodoDisabled"
                                        class="text-[10px] text-red-400 font-bold"
                                        >Indisponível (Conflito)</span
                                    >
                                </div>
                            </div>
                        </label>
            ...
            <div
                class="p-6 border-t border-white/5 bg-[#16161E] relative overflow-hidden"
            >
                <!-- Standard Footer -->
                <div
                    ...
                    :class="
                        showConfirmDelete
                            ? 'translate-y-20 opacity-0 absolute inset-x-6'
