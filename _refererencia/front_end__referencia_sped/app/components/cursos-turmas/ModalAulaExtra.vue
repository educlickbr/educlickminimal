...
import { useCursosTurmasModalAula } from "~/composables/cursos-turmas/useCursosTurmasModalAula";
...
const { saveEvento } = useCursosTurmasModalAula();
...
const save = async () => {
    ...
    try {
        ...
        const response: any = await saveEvento(payload);
        ...
        console.error(e);
        showToast("Erro: " + (e.message || "Falha na requisição"), {
            type: "error",
        });
    } finally {
...
<template>
    ...
    >
        ...
        <div
            class="relative bg-[#1A1B26] border border-white/10 rounded-xl shadow-2xl w-full max-w-md overflow-hidden"
        >
            ...
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label
                            class="block text-xs font-bold text-secondary mb-1.5 uppercase"
                            >Início</label
                        >
                        <input
                ...
                <div>
                    ...
                    <input
                        ...
                        type="text"
                        placeholder="Ex: Reposição, Ensaio Geral..."
                        class="w-full bg-[#0f0f15] border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder:text-secondary/30 focus:border-primary focus:outline-none"
                    />
                </div>
            </div>
