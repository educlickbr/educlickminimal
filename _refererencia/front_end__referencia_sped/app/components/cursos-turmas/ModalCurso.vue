...
import { useCursosTurmasModalCurso } from "~/composables/cursos-turmas/useCursosTurmasModalCurso";
import {
    timeToMinutes,
    minutesToTime,
    calculateTotalHours,
...
const { isLoading, isSaving, loadCurso, saveCurso } =
    useCursosTurmasModalCurso();
...
watchEffect(() => {
    if (form.value.is_irregular) {
        ...
        let totalMins = 0;
        form.value.irregular_meetings.forEach(
            (t) => (totalMins += timeToMinutes(t)),
        );
...
const loadCourseData = async (id: string) => {
    try {
        const data: any = await loadCurso(id);
        ...
        const foundArea = areas.find(
            (a) =>
                a.value
                    .toLowerCase()
                    .normalize("NFD")
                    .replace(/[\u0300-\u036f]/g, "") === data.area,
        );
        if (foundArea) form.value.area = foundArea.value;

        const foundMod = modalidades.find(
            (m) =>
                m.value
                    .toLowerCase()
                    .normalize("NFD")
                    .replace(/[\u0300-\u036f]/g, "") === data.modalidade,
        );
        ...
        if (data.encontros && data.encontros.length > 0) {
            // Populate times
            if (form.value.is_irregular) {
                form.value.irregular_meetings = data.encontros.map((e: any) =>
                    minutesToTime(e.tempo_minutos),
                );
            } else {
                form.value.carga_horaria_encontro = minutesToTime(
                    data.encontros[0].tempo_minutos,
                );
                form.value.irregular_meetings = [];
            }
        } else if (
            !form.value.is_irregular &&
            (data.qtd_minutos_aula || 0) > 0
        ) {
            // Fallback for regular courses without rows in curso_encontros.
            form.value.carga_horaria_encontro = minutesToTime(
                data.qtd_minutos_aula,
            );
        }
    } catch (e: any) {
        ...
        // isLoading handled by composable
    }
};
...
const save = async () => {
    ...
    try {
        ...
        const totalEncontrosCurso = Math.max(totalEncontros.value || 0, 1);
        const qtdPeriodos = Math.max(form.value.periodos || 1, 1);

        const encontroMinutes = form.value.is_irregular
            ? Math.round(totalMinutes / totalEncontrosCurso)
            : timeToMinutes(form.value.carga_horaria_encontro);
        const moduloMinutes = Math.round(
            totalMinutes / Math.max(form.value.modulos || 1, 1),
        );
        const periodoMinutes = Math.round(encontroMinutes / qtdPeriodos);
        ...
        const payload = {
            dados_curso: {
                ...
                // Time structures
                c_carga_horaria_periodo: [0, 0, periodoMinutes],
                c_carga_horaria_encontro: [0, 0, encontroMinutes],
                c_carga_horaria_modulo: [0, 0, moduloMinutes],
        ...
        if (form.value.is_irregular) {
            payload.encontros_curso = form.value.irregular_meetings.map(
                (time, idx) => ({
        ...
        const response = await saveCurso(payload);
        ...
        console.error(e);
        showToast("Erro ao salvar: " + (e.message || e.details), {
            type: "error",
        });
    } finally {
        // isLoading handled by composable
...
<template>
    ...
    >
        ...
        <div
            class="relative bg-[#1A1B26] border border-white/10 rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]"
        >
            <!-- Header -->
            <div
            ...
            >
                <div>
                    ...
                    <p class="text-xs text-secondary">
                        Preencha os dados curriculares
                    </p>
                </div>
                <button
                    @click="emit('close')"
            ...
            <div class="p-6 overflow-y-auto flex-1 custom-scrollbar space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label
                            class="block text-xs font-bold text-secondary mb-2"
                            >Descrição / Ementa Resumida</label
                        >
                        <textarea
                ...
                <div class="space-y-6">
                    <!-- Toggle -->
                    <div
                        class="flex items-center justify-between bg-white/5 p-4 rounded-xl border border-white/5"
                    >
                        <div>
                            <h3 class="text-sm font-bold text-white">
                                Encontros com Irregularidade
                            </h3>
                            <p class="text-[10px] text-secondary">
                                Habilite se os encontros tiverem durações
                                diferentes entre si.
                            </p>
                        </div>
                        <!-- Toggle Switch -->
                        <button
                            ...
                            :class="
                                form.is_irregular ? 'bg-primary' : 'bg-gray-600'
                            "
                        >
                            <div
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <!-- Left: Structure -->
                        <div
                            class="space-y-4 p-4 rounded-xl border border-white/5 bg-[#0f0f15]"
                        >
                            <h4
                                class="text-xs font-bold text-white uppercase tracking-wider mb-4 opacity-70"
                            >
                                Estrutura
                            </h4>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                ...
                                <div class="col-span-2">
                                    <label
                                        class="block text-[10px] font-bold text-secondary mb-1"
                                        >Qtd. Períodos (Períodos no Dia)</label
                                    >
                                    <input
                        ...
                        >
                            ...
                            <div v-if="!form.is_irregular">
                                <label
                                    class="block text-[10px] font-bold text-secondary mb-1"
                                    >Duração Padrão do Encontro (HH:MM)</label
                                >
                                <input
                            ...
                            >
                                ...
                                >
                                    Modo Irregular Ativo: Defina a duração de
                                    cada encontro individualmente abaixo.
                                </p>
                            </div>
                            ...
                            <div>
                                <label
                                    class="block text-[10px] font-bold text-secondary mb-1"
                                    >Carga Horária Total (Calculada)</label
                                >
                                <div
                                    class="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-lg font-mono text-primary font-bold"
                                >
                                    {{ form.carga_horaria_total }}
                                    <span
                                        class="text-xs text-secondary font-normal ml-1"
                                        >horas</span
                                    >
                                </div>
                            </div>
                            ...
                            <div class="pt-2 border-t border-white/5 mt-2">
                                <label
                                    class="block text-[10px] font-bold text-secondary mb-1"
                                    >Carga Horária / Módulo</label
                                >
                                <div
                                    class="text-sm font-mono text-white opacity-80"
                                >
                                    {{ form.carga_horaria_modulo }}
                                    <span
                                        class="text-xs text-secondary font-normal ml-1"
                                        >horas</span
                                    >
                    ...
                    <!-- Irregular Repeater -->
                    <div
                        v-if="form.is_irregular"
                        class="bg-[#0f0f15] border border-white/5 rounded-xl p-4 animate-in fade-in slide-in-from-top-4 duration-300"
                    >
                        <h4
                            class="text-xs font-bold text-white uppercase tracking-wider mb-4 opacity-70 flex items-center justify-between"
                        >
                            <span>Detalhamento dos Encontros</span>
                            <span
                                class="text-[10px] bg-white/10 px-2 py-1 rounded text-white"
                                >{{ form.irregular_meetings.length }} Encontros
                                Total</span
                            >
                        </h4>
                        ...
                        >
                            <div
                                v-for="(time, index) in form.irregular_meetings"
                                :key="index"
                                class="bg-[#1A1B26] p-3 rounded-lg border border-white/5 flex items-center gap-4"
                            >
                                <div class="w-24 shrink-0">
                                    <span
                                        class="text-xs font-bold text-white uppercase"
                                        >Encontro {{ index + 1 }}</span
                                    >
                                </div>
                                ...
                                <div class="flex items-center gap-2 flex-1">
                                    <div class="flex-1">
                                        <label
                                            class="block text-[10px] text-secondary mb-1"
                                            >Horas</label
                                        >
                                        <input
                                            :value="time.split(':')[0]"
                                    ...
                                    <span class="text-white font-bold mt-4"
                                        >:</span
                                    >
                                    <div class="flex-1">
                                        <label
                    ...
                    >
                        ...
                        <div>
                            <p
                                class="text-[10px] font-bold text-secondary uppercase"
                            >
                                CH Média por Encontro
                            </p>
                            <p class="text-sm font-mono text-white">
                                {{
                                    form.is_irregular
                                        ? "Variável"
                                        : form.carga_horaria_encontro
                                }}
                            </p>
                        </div>
                    </div>
