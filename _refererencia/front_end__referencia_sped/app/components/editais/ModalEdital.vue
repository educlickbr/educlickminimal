        ...
        if (newVal) {
            if (props.edital) {
                form.value = {
                    ...
                    data_inicio: props.edital.data_inicio
                        ? props.edital.data_inicio.slice(0, 16)
                        : "",
                    data_fim: props.edital.data_fim
                        ? props.edital.data_fim.slice(0, 16)
                        : "",
                    ...
                    publicado_em: props.edital.publicado_em
                        ? props.edital.publicado_em.slice(0, 16)
                        : "",
                    desativado_em: props.edital.desativado_em
                        ? props.edital.desativado_em.slice(0, 16)
                        : "",
                };
            } else {
                // Reset for new
...
const save = async () => {
    ...
    try {
        ...
        if (file.value) {
            ...
            const uploadRes = await ofetch<{ path: string; fileName: string }>(
                "/api/editais/upload",
                {
                    method: "POST",
                    body: {
        ...
        showToast("Edital salvo com sucesso!", { type: "success" });
        emit("saved");
        emit("close");
    } catch (e: any) {
        console.error(e);
        showToast(e.message || "Erro ao salvar edital.", { type: "error" });
    } finally {
...
<template>
    ...
    >
        ...
        <!-- Modal Container -->
        <div
            class="relative bg-[#1a1b26] border border-white/10 rounded-xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl"
        >
            <!-- Header -->
            <div
                class="flex justify-between items-center p-6 border-b border-white/5 shrink-0"
            >
                    ...
                    >
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="space-y-1">
                                <label
                                    class="text-[10px] text-secondary font-bold uppercase"
                                    >Agendar Desativação (Opcional)</label
                                >
                                <input
                    ...
                    <div class="space-y-1">
                        ...
                        >
                            ...
                            >
                                ...
                                <p
                                    class="text-xs font-bold text-green-400 truncate max-w-full px-4"
                                >
                                    Arquivo Atual:
            ...
            <!-- Footer -->
            <div
                class="flex gap-2 justify-end p-6 border-t border-white/5 bg-[#1a1b26] shrink-0"
            >
                <button
                    @click="emit('close')"
