        ...
        if (newVal) {
            if (props.initialData) {
                form.value = {
                    ...
                    id_categoria:
                        props.initialData.categoria?.id ||
                        props.initialData.id_categoria_produto,
                    id_tipo:
                        props.initialData.tipo?.id ||
                        props.initialData.id_tipo_produto,
                    id_unidade:
                        props.initialData.unidade?.id ||
                        props.initialData.id_unidade,
...
<template>
    ...
    >
        ...
        ></div>
        <div
            class="bg-[#16161E] border border-white/10 rounded-xl w-full max-w-2xl p-6 relative z-10 shadow-2xl max-h-[90vh] overflow-y-auto"
        >
            <h3
                class="text-lg font-bold text-white mb-6 uppercase tracking-wider"
            >
            ...
            <form @submit.prevent="handleSubmit" class="space-y-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div class="col-span-2 space-y-2">
                        ...
                        <input
                            ...
                            type="text"
                            required
                            placeholder="Ex: Câmera Canon 5D Mark IV"
                    ...
                    <div class="space-y-2">
                        <label
                            class="text-[10px] font-black uppercase tracking-widest text-success ml-1"
                            >Qtd. Inicial (Estoque)</label
                        >
                        <input
                            v-model.number="form.quantidade_inicial"
                            type="number"
                            min="0"
                            :disabled="!!initialData"
                            class="w-full bg-[#0f0f15] border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:border-primary focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
                            title="Para adicionar estoque a um produto existente, use a ação rápida no card."
                        />
                    </div>
                    ...
                    <div class="space-y-2">
                        <label
                            class="text-[10px] font-black uppercase tracking-widest text-secondary/60 ml-1"
                            >Valor Unitário (Inicial)</label
                        >
                        <input
                            v-model.number="form.valor_inicial"
                            type="number"
                            step="0.01"
                            min="0"
                            class="w-full bg-[#0f0f15] border border-white/10 rounded-lg px-4 py-3 text-sm text-white focus:border-primary focus:outline-none"
                    ...
                    <div class="col-span-2 space-y-2">
                        <label
                            class="text-[10px] font-black uppercase tracking-widest text-secondary/60 ml-1"
                            >Código de Barras / Patrimônio</label
                        >
                        <input
                ...
                <div
                    class="flex items-center gap-3 pt-4 border-t border-white/5 mt-4"
                >
                    <button
                        type="button"
                        @click="emit('close')"
