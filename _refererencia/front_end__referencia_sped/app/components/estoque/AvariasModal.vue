...
const fetchTiposAvaria = async () => {
    try {
        const data = await $fetch("/api/producao/estoque/tipos-avaria");
        if (data) {
            tipoAvariaOptions.value = data;
        }
    } catch (e) {
        console.error("Erro ao buscar tipos de avaria:", e);
    }
};
...
const fetchAvarias = async () => {
    ...
    try {
        const data = await $fetch(
            `/api/producao/estoque/avarias-por-item?id_item=${props.estoqueItem.id}`,
        );
        if (data) avarias.value = data;
    } catch (e) {
        console.error("Erro ao buscar avarias:", e);
    }
...
const handleSaveAvaria = async () => {
    ...
    try {
        await ofetch("/api/producao/estoque/avaria", {
            method: "POST",
            body: {
...
const hasBlockingAvaria = computed(() => {
    return avarias.value.some((a: any) =>
...
<template>
    ...
    >
        ...
        ></div>
        <div
            class="bg-[#16161E] border border-white/10 rounded-xl w-full max-w-2xl p-6 relative z-10 shadow-2xl max-h-[90vh] overflow-y-auto flex flex-col"
        >
            <!-- Header -->
            <div class="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p class="text-xs text-secondary mt-1">
                        Item: {{ estoqueItem?.produto?.nome }} (ID:
                        {{ estoqueItem?.id.split("-")[0] }})
                    </p>
                </div>
                <button
                ...
                <div
                    v-else-if="hasBlockingAvaria"
                    class="text-xs text-secondary/60 italic border border-white/5 bg-white/5 px-3 py-1.5 rounded"
                >
            ...
            <div
                v-if="newAvariaMode"
                class="bg-black/20 border border-white/5 rounded-xl p-4 mb-6"
            >
                ...
                <form @submit.prevent="handleSaveAvaria" class="space-y-4">
                    <div class="space-y-2">
                        ...
                        <input
                            ...
                            placeholder="Ex: Tela riscada, Botão emperrado..."
                        />
                    </div>
                    <div class="space-y-2">
                        <label
                    ...
                    <div class="flex justify-end gap-2 pt-2">
                        <button
                            type="button"
                            @click="newAvariaMode = false"
                            class="text-xs font-bold text-secondary hover:text-white px-3 py-2"
                        >
                            Cancelar
                        </button>
                        <button
                            type="submit"
                            class="bg-primary hover:bg-primary/90 text-white text-xs font-bold uppercase px-4 py-2 rounded-lg"
                        >
                            Salvar
                        </button>
                    </div>
            ...
            <div class="flex-1 overflow-y-auto">
                ...
                <div
                    v-else-if="avarias.length === 0"
                    class="text-center py-10 text-secondary/50 text-sm"
                >
                    Nenhuma avaria registrada para este item.
                </div>
                <div v-else class="space-y-3">
                    <div
                        v-for="avaria in avarias"
                        :key="avaria.id"
                        class="bg-white/5 rounded-lg border border-white/5 overflow-hidden transition-all duration-300"
                    >
                            ...
                            <div class="flex-1">
                                <div class="flex items-center gap-3 mb-1">
                                    ...
                                    <span
                                        ...
                                        :class="
                                            getStatusColor(avaria.status_reparo)
                                        "
                                    >
                                ...
                                >
                                    <span>{{
                                        new Date(
                                            avaria.data_entrada_avaria,
                                        ).toLocaleDateString()
                                    }}</span>
                                    <span
                                        v-if="avaria.tipo_avaria"
                                        class="text-secondary/50"
                                        >• ID Tipo:
                                        {{ avaria.tipo_avaria }}</span
                                    >
                                </div>
                            </div>
                        ...
                        <div
                            v-if="expandedAvariaId === avaria.id"
                            class="px-4 pb-4 pt-0 border-t border-white/5 bg-black/20"
                            @click.stop
                        >
                            <div class="pt-4 space-y-4">
                                ...
                                >
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        >
                                        <input
                                            v-model="editingForm.descricao"
                                            type="text"
                                            class="w-full bg-[#0f0f15] border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-primary focus:outline-none placeholder-secondary/30"
                                        />
                                    </div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    >
                                    <textarea
                                        v-model="editingForm.observacoes"
                                        rows="2"
                                        class="w-full bg-[#0f0f15] border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-primary focus:outline-none placeholder-secondary/30 resize-none"
                                    ></textarea>
                                </div>
                                ...
                                <div class="flex justify-end gap-2 pt-2">
                                    <button
                                        @click="expandedAvariaId = null"
                                        class="text-xs font-bold text-secondary hover:text-white px-3 py-2"
                                    >
                                        Cancelar
                                    </button>
                                    <button
                                        @click="handleUpdateAvaria(avaria.id)"
                                        class="bg-primary hover:bg-primary/90 text-white text-xs font-bold uppercase px-4 py-2 rounded-lg"
                                    >
                                        Salvar Alterações
                                    </button>
