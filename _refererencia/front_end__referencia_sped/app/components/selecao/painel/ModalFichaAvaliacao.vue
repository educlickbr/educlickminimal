...
import { usePainelListas } from "~/composables/selecao/usePainelListas";
...
const {
    isListLoading,
    fetchListData: _fetchListData,
    clearCache,
    openPrintDocument,
    printListaPresenca: _printListaPresenca,
    printListaNumerica: _printListaNumerica,
    printClassListaPresenca: _printClassListaPresenca,
    printClassListaNumerica: _printClassListaNumerica,
} = usePainelListas();

const isLoading = ref(false);
const activeTab = ref<"config" | "preview">("config");
const dataInicio = ref("");
const incluirVagasComplementares = ref(false);
...
const areaLabel = computed(() => {
    const area = (props.area || "").trim().toLowerCase();
    if (area === "regulares") return "Cursos Regulares";
    if (area === "extensao" || area === "extensão") return "Extensão";
    if (area === "cursos_livres" || area === "cursos livres")
        return "Cursos Livres";
    return props.area || "";
});

const tituloProcessoFicha = computed(() => {
    const sufixo = incluirVagasComplementares.value
        ? " vagas complementares"
        : "";
    return `Processo seletivo${sufixo} ${props.anoSemestre || ""}`.trim();
});

const tituloProcessoLista = computed(() =>
    `Processo Seletivo ${areaLabel.value} ${nomeProcesso.value}`.trim(),
);

// Wrappers para o composable de listas
const fetchListData = () =>
    _fetchListData(props.area, props.anoSemestre, dataInicio.value);
const printListaPresenca = async () => {
    await fetchListData();
    _printListaPresenca(tituloProcessoLista.value);
};
const printListaNumerica = async () => {
    await fetchListData();
    _printListaNumerica(tituloProcessoLista.value);
};
const printClassListaPresenca = (turma: any) =>
    _printClassListaPresenca(
        turma,
        tituloProcessoLista.value,
        props.area,
        props.anoSemestre,
        dataInicio.value,
    );
const printClassListaNumerica = (turma: any) =>
    _printClassListaNumerica(
        turma,
        tituloProcessoLista.value,
        props.area,
        props.anoSemestre,
...
const saveParams = async (turma: any) => {
    ...
    turma.isSaving = true;
    try {
        await ($fetch as any)("/api/selecao/ficha-avaliacao-config", {
            method: "POST",
            body: {
        ...
        turma.lastSaved = new Date().toLocaleTimeString("pt-BR", {
            hour: "2-digit",
            minute: "2-digit",
        });
    } catch (e) {
...
const printPDF = (items: any[] = []) => {
    ...
    openPrintDocument(generateFullHTML(turmasToPrint));
};
...
const generateBody = (items: any[]) => {
    ...
    return items
        .map((t) => {
            ...
            // Rodape
            const textoRodapeRaw =
                `Perfil do Curso de ${t.nome_turma}:\n${t.custom_rodape || ""}`.trim();
            const [rodapeTitulo, ...rodapeTextoArray] =
                textoRodapeRaw.split("\n");
                ...
                <div class="pagina">
                  ...
                  <div class="bloco-topo">
                    <div class="linha-topo">
                      <div class="badge-area">${areaLabel.value}</div>
                      <div class="topo-centro">
                        <div class="linha1">${tituloProcessoFicha.value} ${subtituloProcesso}</div>
...
<template>
    <div v-if="isOpen" class="relative z-50">
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
            ...
            >
                <div
                    class="relative w-full max-w-4xl transform rounded-none md:rounded-xl bg-[#16161E] border border-white/10 text-left shadow-xl transition-all flex flex-col max-h-[90vh]"
                >
                    <div
                    ...
                    >
                        <div class="flex flex-col gap-1">
                            <h3 class="text-xl font-bold text-white">
                                ...
                                <span
                                    class="block text-sm text-secondary font-normal mt-1 capitalize"
                                    >{{ area }} | {{ anoSemestre }}</span
                                >
                            </h3>
                        </div>
                        <div class="flex items-center gap-3">
                            <!-- Date Filter -->
                            <div
                                class="flex items-center gap-2 bg-[#0f0f15] border border-white/10 rounded-lg p-1.5 pl-3"
                            >
                                <span
                            ...
                            <button
                                @click="$emit('close')"
                                class="text-secondary hover:text-white transition-colors ml-4"
                            >
                                <svg
                                    class="w-6 h-6"
                                    fill="none"
                    ...
                    <div class="flex-1 overflow-y-auto p-6 custom-scrollbar">
                        ...
                        >
                            <div>
                                <h4
                                    class="text-sm font-bold text-white mb-4 uppercase tracking-wider"
                                >
                                    Configuração por Turma ({{ turmas.length }})
                                </h4>
                                <div
                                    class="mb-4 flex items-center justify-between bg-white/5 border border-white/10 rounded-lg px-4 py-3"
                                >
                                    <div class="pr-4">
                                        <p class="text-sm font-bold text-white">
                                            Incluir "vagas complementares" no
                                            título
                                        </p>
                                        <p
                                            class="text-xs text-secondary mt-0.5"
                                        >
                                            Desligado: Processo seletivo
                                            {{ anoSemestre }} | Ligado: Processo
                                ...
                                <div class="space-y-3">
                                    ...
                                    >
                                                ...
                                                <div
                                                    v-if="
                                                        (turma.alunos || [])
                                                            .length === 0
                                                    "
                                                    class="flex items-center gap-1 mt-1 text-xs text-yellow-500 font-bold"
                                                >
                                                    <svg
                                                        class="w-3 h-3"
                                                        fill="none"
                                        ...
                                        <div
                                            v-show="turma.isExpanded"
                                            class="p-4 pt-0 border-t border-white/10 mt-2 space-y-4"
                                        >
                                            <!-- Perguntas -->
                                            <div class="space-y-4 mt-4">
                                                <div v-for="i in 3" :key="i">
                                                    <label
                                                        class="block text-xs font-bold text-secondary mb-1"
                                                        >Pergunta {{ i }}</label
                                                    >
                                                    <div class="flex gap-2">
                                                        ...
                                                        <button
                                                            ...
                                                            "
                                                            class="text-primary hover:text-primary-600 transition-colors p-1"
                                                            title="Salvar"
                                                        >
                                                            <svg
                                                                class="w-5 h-5"
                                                    ...
                                                    <div class="mt-1 h-4">
                                                        <span
                                                            v-if="
                                                                turma.isSaving
                                                            "
                                                            class="text-xs text-yellow-500 animate-pulse"
                                                            >Aguarde...</span
                                                        >
                                                        <span
                                                            v-else-if="
                                                ...
                                                <div>
                                                    ...
                                                    <div class="flex gap-2">
                                                        ...
                                                        <button
                                                            ...
                                                            "
                                                            class="text-primary hover:text-primary-600 transition-colors p-1 self-start"
                                                            title="Salvar"
                                                        >
                                                            <svg
                                                                class="w-5 h-5"
                        ...
                        >
                            <div
                                v-for="turma in turmas"
                                :key="turma.id_turma"
                                v-show="(turma.alunos || []).length > 0"
                                class="bg-white/5 border border-white/10 rounded-lg overflow-hidden"
                            >
                                <div
                                    class="w-full flex justify-between items-center p-5 hover:bg-white/5 transition-colors text-left group"
                                >
                                    <button
                                        @click="
                                            turma.isPreviewExpanded =
                                                !turma.isPreviewExpanded
                                        "
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <span
                                                class="text-xs text-secondary w-fit bg-white/5 px-2 py-0.5 rounded"
                                                >{{ turma.turno }}</span
                                            >
                                        </div>
                                        <div class="flex items-center gap-4">
                                            <span
                                                class="text-xs font-bold text-white bg-primary/20 text-primary px-2 py-1 rounded"
                                            >
                                                {{
                                                    (turma.alunos || []).length
                                                }}
                                    ...
                                    <div
                                        class="ml-2 border-l border-white/10 pl-4 flex flex-col gap-2"
                                    >
                                        <button
                                            @click.stop="printClass(turma)"
                                            class="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-bold bg-primary hover:bg-primary-600 text-white rounded-lg transition-colors"
                                            title="Imprimir fichas desta turma"
                                        >
                                            <svg
                                                class="w-4 h-4"
                                                fill="none"
                                        ...
                                        <button
                                            type="button"
                                            @click.stop="
                                                printClassListaPresenca(turma)
                                            "
                                            :disabled="isListLoading"
                                            class="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-bold bg-primary hover:bg-primary-600 text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                            title="Imprimir lista desta turma"
                                        >
                                            <svg
                                                class="w-4 h-4"
                                                fill="none"
                                            ...
                                            >
                                                <path
                                                    ...
                                                    stroke-linejoin="round"
                                                    stroke-width="2"
                                                    d="M9 12h6m-6 4h6m-7 4h8a2 2 0 002-2V6a2 2 0 00-2-2H8a2 2 0 00-2 2v12a2 2 0 002 2zm8-14H8"
                                                ></path>
                                            </svg>
                                            {{
                                                isListLoading
                                        ...
                                        <button
                                            type="button"
                                            @click.stop="
                                                printClassListaNumerica(turma)
                                            "
                                            :disabled="isListLoading"
                                            class="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-bold bg-primary hover:bg-primary-600 text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                            title="Imprimir lista numérica desta turma"
                                        >
                                            <svg
                                                class="w-4 h-4"
                                                fill="none"
                                ...
                                <div
                                    v-show="turma.isPreviewExpanded"
                                    class="p-5 pt-0 border-t border-white/10"
                                >
                                    <div class="space-y-1 mt-4">
                                        <div
                                            v-for="(aluno, idx) in turma.alunos"
                                            :key="idx"
                                            class="flex gap-3 text-sm text-secondary-300 py-1 border-b border-white/5 last:border-0"
                                        >
                                            <span
                                                class="w-8 text-right font-mono opacity-50"
                                                >{{ Number(idx) + 1 }}.</span
                                            >
                                        ...
                                        <div
                                            ...
                                            "
                                            class="text-xs text-secondary/5 italic py-2"
                                        >
                    ...
                    >
                        <button
                            @click="$emit('close')"
                            class="px-4 py-2 bg-white/5 hover:bg-white/10 text-secondary hover:text-white text-sm font-semibold rounded-lg transition-colors"
                        >
                            Cancelar
                        </button>
                        <template v-if="activeTab === 'preview'">
                            <button
                                @click="printPDF()"
                                class="px-4 py-2 bg-primary hover:bg-primary-600 text-white text-sm font-semibold rounded-lg transition-colors flex items-center gap-1.5"
                            >
                                <svg
                                    class="w-4 h-4"
                                    fill="none"
                            ...
                            <button
                                type="button"
                                @click="printListaPresenca()"
                                :disabled="isListLoading"
                                class="px-4 py-2 bg-primary hover:bg-primary-600 text-white text-sm font-semibold rounded-lg transition-colors flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                <svg
                                    class="w-4 h-4"
                                    fill="none"
                                ...
                                {{
                                    isListLoading
                                        ? "Gerando Listas..."
                                        : "Imprimir Listas Presença"
                                }}
                            </button>
                            <button
                                type="button"
                                @click="printListaNumerica()"
                                :disabled="isListLoading"
                                class="px-4 py-2 bg-primary hover:bg-primary-600 text-white text-sm font-semibold rounded-lg transition-colors flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                <svg
