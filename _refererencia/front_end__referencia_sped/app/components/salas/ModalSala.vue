...
const handleSubmit = async () => {
    ...
    if (!/^#[0-9A-F]{6}$/i.test(form.value.cor)) {
        showToast("Cor inválida. Use formato Hex (ex: #FFFFFF)", {
            type: "info",
        });
        return;
    }
    ...
    try {
        ...
        await ofetch("/api/producao/salas/upsert", {
            method: "POST",
            body: payload,
        });
...
<template>
    ...
    >
        <div
            class="bg-[#1A1B26] border border-white/10 rounded-xl w-full max-w-md shadow-2xl"
        >
            <div
            ...
            <div class="p-6 border-t border-white/5 flex justify-end gap-3">
                <button
                    @click="$emit('close')"
                    class="px-4 py-2 text-secondary hover:text-white transition-colors text-sm font-bold"
                >
                    Cancelar
                </button>
                <button
