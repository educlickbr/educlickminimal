...
const handleUpdate = async () => {
    ...
    if (!session) {
        showToast('Sessão expirada ou inválida. Solicite um novo link.', { type: 'error' })
        return
    }
...
onMounted(async () => {
    // Check if we have a session (recovery flow logs the user in)
    const { data: { session } } = await supabase.auth.getSession()
    
    if (session) {
        isVerifying.value = false
    } else {
        // Wait for potential auto-login from hash
        ...
        setTimeout(async () => {
            ...
            if (!session) {
                ...
                if (hash.includes('error=')) {
                     showToast('Link de recuperação inválido ou expirado.', { type: 'error' })
                }
            }
            isVerifying.value = false
        }, 2000)
