...
const handleSignup = async () => {
    ...
    try {
        ...
        // 2) Persistir o perfil via BFF
        const { success } = await $fetch<{ success: boolean }>('/api/cadastro/submit', {
            method: 'POST',
            body: {
                credentials: {
                    nome: nome.value,
        ...
        errorMsg.value = err?.data?.statusMessage || err.message || 'Erro ao realizar inscrição'
    ...
    <div class="w-full max-w-md relative z-10">
        ...
        <div class="bg-transparent md:bg-div-15 md:backdrop-blur-2xl border-none md:border md:border-white/5 rounded-none md:rounded-xl p-6 md:p-12 shadow-none md:shadow-2xl">
            ...
            <div class="mt-8 text-center space-y-4">
                ...
                <NuxtLink to="/login" class="inline-block text-[10px] font-black uppercase tracking-widest text-white/80 bg-white/5 border border-white/10 px-8 py-4 rounded-lg hover:bg-white/10 hover:text-white transition-all">
                    Entrar
                </NuxtLink>
            </div>
