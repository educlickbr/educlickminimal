...
/**
 * Converts a date to the HTML datetime-local input format (YYYY-MM-DDTHH:mm)
 * using 'America/Sao_Paulo' timezone.
 * This is the standard way to populate <input type="datetime-local"> fields.
 */
export const toInputDateTimeLocal = (
    date: string | Date | null | undefined,
): string => {
    if (!date) return "";
    try {
        ...
        // Format: YYYY-MM-DDTHH:mm (NO seconds, NO timezone offset for datetime-local)
        return formatInTimeZone(d, TIMEZONE, "yyyy-MM-dd'T'HH:mm");
    } catch (e) {
        console.error("Error converting to input datetime-local:", e);
        return "";
    }
};
