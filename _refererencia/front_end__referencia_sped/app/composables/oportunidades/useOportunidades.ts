...
// Composable: estado e fetch de oportunidades + download + delete
export function useOportunidades() {
  ...
  const fetchOportunidades = async (anoSemestre: string) => {
    ...
    try {
      ...
      console.error(e);
      toast.showToast("Erro ao carregar oportunidades.", { type: "error" });
    } finally {
      isLoading.value = false;
    }
  };

  const handleDelete = async (item: any, anoSemestre: string) => {
    if (!confirm("Tem certeza que deseja excluir esta oportunidade?")) return;

    try {
      await ofetch(`/api/bolsas/oportunidades/${item.id}`, {
        method: "DELETE",
      });
      toast.showToast("Oportunidade excluída.", { type: "success" });
      fetchOportunidades(anoSemestre);
    } catch (e: any) {
      console.error(e);
      toast.showToast("Erro ao excluir oportunidade.", { type: "error" });
    }
  };
  ...
  return { oportunidades, isLoading, fetchOportunidades, handleDelete, downloadFile };
}
