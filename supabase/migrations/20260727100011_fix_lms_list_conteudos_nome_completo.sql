-- ============================================================
-- Migration: fix_lms_list_conteudos_nome_completo
-- Data: 2026-07-27
-- Descrição: Corrige a RPC lms_list_conteudos para usar
--            nome_completo (coluna real de user_expandido)
--            ao invés de nome + sobrenome (que não existem).
--            Mesma correção já aplicada em outras RPCs
--            (ver migration 20260325165731).
-- ============================================================

DROP FUNCTION IF EXISTS public.lms_list_conteudos(
    p_id_entidade UUID,
    p_pagina INTEGER,
    p_limite INTEGER,
    p_busca TEXT,
...
BEGIN
    WITH base AS (
        SELECT
            c.*,
            COALESCE(ux.nome_completo, ux.email) AS criado_por_nome,
