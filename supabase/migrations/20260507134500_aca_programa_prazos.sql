-- ============================================================
-- Migration: aca_programa_prazos
-- Data: 2026-05-07
-- Descrição: Adiciona colunas de prazos de processo seletivo e matrícula
--            à tabela aca_programa e atualiza RPCs correspondentes.
-- ============================================================

-- 1. Adicionar colunas de prazos na tabela aca_programa
ALTER TABLE public.aca_programa 
ADD COLUMN IF NOT EXISTS processo_seletivo_inicio TIMESTAMPTZ NULL,
ADD COLUMN IF NOT EXISTS processo_seletivo_fim TIMESTAMPTZ NULL,
ADD COLUMN IF NOT EXISTS matricula_inicio TIMESTAMPTZ NULL,
ADD COLUMN IF NOT EXISTS matricula_fim TIMESTAMPTZ NULL;

-- 2. Atualizar RPC: aca_upsert_programa para suportar as novas colunas
...
-- 3. Atualizar RPC: aca_create_programas_lote para suportar as novas colunas
...
BEGIN
    ...
    IF p_estrategia = 'unica' THEN
        INSERT INTO public.aca_programa (
            id_entidade, id_curso, descricao, criado_por, id_area,
            processo_seletivo_inicio, processo_seletivo_fim, matricula_inicio, matricula_fim
        )
        VALUES (
            p_id_entidade, p_id_curso, p_descricao, p_usuario_id, v_id_area,
            p_processo_seletivo_inicio, p_processo_seletivo_fim, p_matricula_inicio, p_matricula_fim
        )
