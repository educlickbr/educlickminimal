-- ============================================================
-- Migration: drop_and_recreate_com_produto_rpcs
-- Data: 2026-07-03
-- Descrição: Dropa todas as overloads das RPCs de com_produto
--            e recria com a assinatura correta (sem tipo_recorrencia)
-- ============================================================

-- ============================================================
-- 1. Dropar funções antigas (com assinatura explícita)
-- ============================================================
DROP FUNCTION IF EXISTS public.com_get_produtos(uuid, integer, integer, uuid);
DROP FUNCTION IF EXISTS public.com_upsert_produto(uuid, uuid, uuid, text, text, text, boolean, uuid);
DROP FUNCTION IF EXISTS public.com_upsert_produto(uuid, uuid, uuid, text, text, boolean, uuid);
DROP FUNCTION IF EXISTS public.com_delete_produto(uuid, uuid);
...
-- ============================================================
-- 3. com_upsert_produto (sem tipo_recorrencia)
-- ============================================================
...
-- ============================================================
-- 4. com_delete_produto (inalterada, apenas recriada pra limpeza)
-- ============================================================
CREATE OR REPLACE FUNCTION public.com_delete_produto(
    p_id UUID,
    p_id_entidade UUID
)
RETURNS JSONB
...
AS $$
DECLARE
    v_ofertas_count INTEGER;
BEGIN
    IF p_id IS NULL OR p_id_entidade IS NULL THEN
        RAISE EXCEPTION 'ID do produto e entidade são obrigatórios';
    END IF;

    SELECT COUNT(*) INTO v_ofertas_count
    FROM public.com_oferta
    WHERE id_produto = p_id AND id_entidade = p_id_entidade;

    IF v_ofertas_count > 0 THEN
        RAISE EXCEPTION 'Não é possível excluir: existem % oferta(s) vinculadas a este produto', v_ofertas_count;
    END IF;

    DELETE FROM public.com_produto
    ...
    RETURN jsonb_build_object('success', true, 'message', 'Produto excluído com sucesso');
END;
$$;
