-- ============================================================
-- Migration: fix_lms_list_curriculo_outer_refs
-- Data: 2026-07-27
-- Descrição: Corrige referências externas a aliases de
--            subqueries (c_cycle, cal) nos ORDER BY dos
--            jsonb_agg externos do lms_list_curriculo.
--            Esses aliases não são visíveis fora da subquery
--            — precisam referenciar o alias da subquery (ciclo_json, aula_json).
--            Também adiciona data_ini ao SELECT do ciclo_json.
-- ============================================================
...
BEGIN
    ...
    SELECT jsonb_agg(jsonb_build_object(
        ...
        'conteudos', COALESCE((
            SELECT jsonb_agg(sub ORDER BY sub.titulo) FROM (
                SELECT DISTINCT ON (d.id_conteudo)
    ...
    SELECT jsonb_agg(jsonb_build_object(
        ...
        'ciclos', COALESCE((
            SELECT jsonb_agg(ciclo_json ORDER BY ciclo_json.data_ini) FROM (
                SELECT
                    c_cycle.id,
                    COALESCE(c_cycle.descricao, m.nome_modulo) AS nome,
                    c_cycle.data_ini,
                    COALESCE((
                    ...
                    COALESCE((
                        SELECT jsonb_agg(aula_json ORDER BY aula_json.dt_hora_ini) FROM (
                            SELECT
                                cal.id,
                                'Aula ' || to_char(cal.dt_hora_ini, 'DD/MM') AS nome,
                                cal.dt_hora_ini,
                                COALESCE((
    ...
    ) ORDER BY m.nome_modulo) INTO v_modulos
    FROM modulos m;
