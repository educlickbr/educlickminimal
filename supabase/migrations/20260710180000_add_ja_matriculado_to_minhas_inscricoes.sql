-- ============================================================
-- Fix: aca_get_minhas_inscricoes — adiciona ja_matriculado
-- Indica se o aluno já possui matrícula no programa
-- ============================================================
...
BEGIN
    ...
    SELECT jsonb_agg(sub ORDER BY sub.data_inscricao DESC) INTO v_itens
    FROM (
        SELECT
            ...
            a.nome_area,
            of.id AS id_oferta,
            of.slug AS oferta_slug,
            (SELECT COUNT(*) > 0 FROM public.aca_matricula WHERE id_programa = prog.id AND id_usuario = v_user_expandido_id) AS ja_matriculado,
            (
        ...
        LEFT JOIN public.com_produto cp ON cp.id_programa = prog.id AND cp.is_ativo = true
        LEFT JOIN public.com_oferta of ON of.id_produto = cp.id AND of.is_ativa = true
        WHERE ins.id_usuario = v_user_expandido_id
