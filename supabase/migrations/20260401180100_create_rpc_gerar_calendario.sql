-- ============================================================
-- Funções do Calendário: Simulação e Geração (SECURITY INVOKER)
-- ============================================================

-- 1. FUNÇÃO DE SIMULAÇÃO (apenas calcula sem gravar)
-- p_dias_semana = [{ "dia_sem": 1, "hora_ini": "19:00", "hora_fim": "22:00" }, ...] (0=Domingo, 6=Sábado)
-- p_dias_extras = [{ "data": "2026-05-15", "hora_ini": "08:00", "hora_fim": "12:00", "observacoes": "Reposição" }]
...
BEGIN
    ...
    WHILE v_carga_restante > 0 LOOP
        ...
        -- 2. Verifica se a data atual é feriado (se for extra, o passo acima reporá, senão, ele pula feriados regulares)
        SELECT nome INTO v_feriado_nome 
        FROM public.aca_feriado 
        WHERE (id_entidade = p_id_entidade OR id_entidade IS NULL) 
          AND (
            data = v_data_atual 
...
-- 2. FUNÇÃO DE GERAÇÃO DEFINITIVA (Grava no banco após upsert do ciclo, regras semanais e extras já estarem validadas/criadas)
...
BEGIN
    ...
    -- Chamar simulação
    SELECT public.aca_simular_calendario_ciclo(
