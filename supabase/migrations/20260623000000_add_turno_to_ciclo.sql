-- ============================================================
-- Migration: Adicionar coluna turno em aca_ciclo
-- Data: 2026-06-23
-- Descrição: Adiciona tipo de turno (Matutino, Vespertino, Noturno,
--            Matutino/Vespertino, Vespertino/Noturno, Integral)
--            ao ciclo acadêmico para uso em cards de inscrição e oferta.
-- ============================================================
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_type WHERE typname = 'tipo_turno'
    ) THEN
...
-- 2. Adicionar a coluna à tabela
ALTER TABLE public.aca_ciclo ADD COLUMN IF NOT EXISTS turno public.tipo_turno;

-- 3. Atualizar a RPC aca_upsert_ciclo_v1 para aceitar o novo campo
DROP FUNCTION IF EXISTS public.aca_upsert_ciclo_v1(
    UUID, UUID, UUID, UUID, TEXT, VARCHAR, DATE, DATE, UUID
);
...
CREATE OR REPLACE FUNCTION public.aca_upsert_ciclo_v1(
    ...
    p_turno public.tipo_turno DEFAULT NULL,
...
BEGIN
    ...
    ON CONFLICT (id) DO UPDATE SET
        ...
        turno          = COALESCE(p_turno, aca_ciclo.turno),
