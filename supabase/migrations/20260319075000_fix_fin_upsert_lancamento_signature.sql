-- MIGRATION: Atualiza a assinatura da RPC fin_upsert_lancamento para incluir p_criado_por
-- Data: 2026-03-19
-- Motivo: Corrigir erro de FK na coluna criado_por que exige referência à tabela user_expandido

-- É necessário o DROP pois a assinatura (número de parâmetros) mudou
...
GRANT EXECUTE ON FUNCTION public.fin_upsert_lancamento(uuid, uuid, uuid, uuid, uuid, text, numeric, text, date, boolean, integer, boolean, uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fin_upsert_lancamento(uuid, uuid, uuid, uuid, uuid, text, numeric, text, date, boolean, integer, boolean, uuid, uuid) TO service_role;
