-- ============================================================
-- RPC: aca_calcular_cronograma_aulas  (FIX feriados multi-entidade)
-- Data: 2026-04-22
-- Descrição:
--   A consulta de feriados agora usa as entidades do JWT do usuário
--   autenticado, e não apenas a entidade passada como parâmetro.
--   Isso permite que feriados cadastrados em qualquer entidade
--   acessível ao usuário (polo, empresa, etc.) sejam respeitados
--   na simulação e geração do cronograma.
--   SECURITY INVOKER: RLS já garante o isolamento entre clientes.
-- ============================================================
...
BEGIN
    -- 0. Verifica se o módulo existe e pertence à entidade
    SELECT EXISTS(
    ...
    -- 1. Carga horária = soma dos componentes vinculados ao módulo
    ...
    WHILE v_carga_restante > 0 LOOP
        ...
        -- A. Verifica se a data é um dia extra cadastrado (tem prioridade máxima)
        ...
        -- B. Verifica feriado ou recesso
        --    Busca em TODAS as entidades do usuário autenticado (via JWT),
        --    além de feriados globais (is_global=true) e sem entidade (id_entidade IS NULL).
        --    SECURITY INVOKER + RLS garantem que só entidades do próprio cliente são visíveis.
        ...
        -- C. Verifica evento que sobrescreve o calendário (da entidade do ciclo)
        ...
        -- D. Verifica grade semanal regular
        ...
        -- Nenhuma regra se aplica: avança o dia
        v_data_atual := v_data_atual + INTERVAL '1 day';
    END LOOP;
